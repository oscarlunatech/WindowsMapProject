...
//
// stylesheet must have been built against this same dataset (see
...
// instead of one per feature. layerCaches must be the same size as
// dataset.layers(), one entry per layer in order. Returns the total number
// of features actually drawn (for a "features drawn"/"culled" overlay - see
// Viewer::onPaint).
//
...
// the per-frame path: like layerCaches, build it once when the dataset loads
// (style::Stylesheet's constructor is O(features)) and pass the same one
// every frame. Throws RenderError if it wasn't built against this dataset.
//
