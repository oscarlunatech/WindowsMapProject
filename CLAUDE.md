...
**Done: phases 1–10. Next: Phase 11 (raster).**
...
| **11** | **Raster** — GDAL raster read, georeferenced display, band selection + stretch. First non-vector data path in the codebase | next |
...
Single test / filtered run (Catch2 tags, e.g. `[render]`, `[dataset]`, `[geometry]`, `[viewport]`, `[index]`, `[simplify]`, `[layer_cache]`, `[style]`, `[predicates]`, `[identify]`, `[map]`):
```
