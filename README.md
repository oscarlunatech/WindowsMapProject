...
Status: **Phase 12 (C++/CLI interop) complete** — 12 of 21 phases to 1.0. See [DECISIONS.md](DECISIONS.md) and [BENCHMARKS.md](BENCHMARKS.md) for the running log.
...
```
Cartograph.Shell    (C#, WPF, MVVM)          — layer list, toolbar, identify panel, attribute table   [Phase 13]
...
`Cartograph.Interop` is marshalling and nothing else — Core's types converted to .NET shapes, Core's exceptions translated into a matching managed hierarchy, and strings crossing as UTF-8 in both directions. If a member there does anything but convert and forward, it belongs on the other side of the boundary.

## Building

Prerequisites: Visual Studio 2022+ with the "Desktop development with C++" workload, [vcpkg](https://github.com/microsoft/vcpkg) with `VCPKG_ROOT` set, and CMake 3.25+ on PATH. From Phase 12, also the .NET 10 SDK — which is where `ijwhost.lib` and the reference assemblies for C++/CLI come from, so no extra Visual Studio component is needed.
...
The managed projects (`Cartograph.Interop`, and from Phase 13 the shell) are built by MSBuild rather than CMake — CMake's C++/CLI support is Visual-Studio-generator only, and it cannot build a WPF project at all. Build them after the native tree:

```
scripts\build-managed.ps1
```

That builds `Cartograph.Interop` and its C# boundary test suite into `build\managed\x64-debug\`, then runs the tests. It needs no NuGet restore. The link line it uses is generated by the CMake configure step, so it never has to be kept in sync by hand.
...
Built in ordered phases; each is buildable and tested before the next starts. **12 of 21 phases done.** The authoritative per-phase plan — scope, ordering constraints, and how thin each pillar's 1.0 slice is — lives in [CLAUDE.md](CLAUDE.md), so it stays in one place rather than drifting between two.
...
| 12 | **The interop boundary** — C++/CLI marshalling layer over Core | done |
| 13–14 | **The shell** — WPF: map surface, layer list, toolbar, identify panel, attribute table | next |
