...
Status: **Phase 8 (identify) complete** — 8 of 21 phases to 1.0. See [DECISIONS.md](DECISIONS.md) and [BENCHMARKS.md](BENCHMARKS.md) for the running log.
...
Built in ordered phases; each is buildable and tested before the next starts. **8 of 21 phases done.** The authoritative per-phase plan — scope, ordering constraints, and how thin each pillar's 1.0 slice is — lives in [CLAUDE.md](CLAUDE.md), so it stays in one place rather than drifting between two.

| Phases | | |
|---|---|---|
| 1–7 | **Foundations** — data model, rendering, live window, performance, threading, reprojection, symbology | done |
| 8–11 | **A real map model** — identify ✓, then multi-layer, selectable display CRS, raster display | in progress |
